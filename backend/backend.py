...
def get_db_transaction():
    ...
    if IS_PRODUCTION and DB_URL:
        ...
        try:
            ...
            conn.autocommit = (
                False  # CRITICAL: Disable autocommit for manual transactions
            )
...
def format_cursor_row(row, column_names):
    ...
    if isinstance(row, dict) or hasattr(row, "get"):
        ...
        # SQLite returns tuples
        return {
            col: row[i] if i < len(row) else None for i, col in enumerate(column_names)
        }
    ...
    try:
        with get_db() as conn:
            ...
            if IS_PRODUCTION and DB_URL:
                ...
                try:
                    ...
                    def column_exists(table_name, column_name):
                        try:
                            ...
                            try:
                                c.execute(
                                    f"ALTER TABLE {table_name} ADD COLUMN {column_name}_test_temp TEXT"
                                )
                                c.execute(
                                    f"ALTER TABLE {table_name} DROP COLUMN {column_name}_test_temp"
                                )
                    ...
                    if not column_exists("events", "start_time"):
                        c.execute(
                            "ALTER TABLE events ADD COLUMN start_time TEXT DEFAULT '12:00'"
                        )
                    ...
                    if not column_exists("events", "interest_count"):
                        c.execute(
                            "ALTER TABLE events ADD COLUMN interest_count INTEGER DEFAULT 0"
                        )
                    ...
                    if not column_exists("events", "secondary_category"):
                        c.execute(
                            "ALTER TABLE events ADD COLUMN secondary_category TEXT"
                        )
                    ...
                    if not column_exists("events", "verified"):
                        c.execute(
                            "ALTER TABLE events ADD COLUMN verified BOOLEAN DEFAULT FALSE"
                        )
                    ...
                    if not column_exists("events", "is_premium_event"):
                        c.execute(
                            "ALTER TABLE events ADD COLUMN is_premium_event BOOLEAN DEFAULT FALSE"
                        )
                ...
                conn.commit()
                logger.info(
                    "✅ Interest, view tracking, privacy requests, and event reports tables created/verified"
                )
            else:
                ...
                if "events" not in [
                    ...
                    if "start_time" not in columns:
                        ...
                        if "time" in columns:
                            ...
                            logger.info(
                                "Added start_time column and migrated from time"
                            )
                        else:
                            c.execute(
                                '''ALTER TABLE events ADD COLUMN start_time TEXT DEFAULT "12:00"'''
...
class AutomatedTaskManager:
    ...
    async def generate_sitemap_automatically(self):
        ...
        try:
            ...
            self.task_status["sitemap_generation"][
                "last_run"
    ...
    async def refresh_event_data(self):
        ...
        try:
            ...
            self.task_status["event_refresh"][
                "last_run"
            ] = datetime.utcnow().isoformat()
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
                ...
                if event.get(date_field):
                    try:
                        parsed_date = datetime.fromisoformat(
                            str(event[date_field]).replace("Z", "+00:00")
                        )
  ...
  <url>
    ...
    <priority>0.85</priority>
        ...
        logger.info(
            f"Generated sitemap with {url_count} URLs from {event_count} future events"
        )
        return sitemap
        ...
        async with httpx.AsyncClient() as client:
            for url in urls:
                try:
                    ...
                    logger.info(
                        f"✅ Pinged search engine: {url} - Status: {response.status_code}"
                    )
                except Exception as e:
                    logger.warning(
                        f"⚠️ Failed to ping search engine: {url} - Error: {str(e)}"
                    )
    ...
    async def cleanup_expired_events(self):
        ...
        try:
            ...
            with get_db() as conn:
                ...
                archive_cutoff_datetime = current_datetime - timedelta(
                    days=32
                )  # 32-day archive
                ...
                logger.info(
                    f"Archive cutoff datetime: {archive_cutoff_datetime.isoformat()}"
                )
                logger.info(
                    f"Archive cutoff date: {archive_cutoff} (events before this will be deleted)"
                )
                ...
                if not expired_events:
                    logger.info(
                        "✅ No expired events found beyond 32-day archive period"
                    )
                    return

                logger.info(
                    f"📊 Found {len(expired_events)} events beyond 32-day archive:"
                )
                for event in expired_events[:5]:  # Log first 5
                    ...
                    logger.info(
                        f"  - ID {event_data['id']}: '{event_data['title']}' (Date: {event_data['date']})"
                    )
                ...
                expired_ids = [
                    event[0] if not isinstance(event, dict) else event["id"]
                ...
                try:
                    ...
                    for event_id in expired_ids:
                        ...
                        if placeholder == "?":
                            ...
                            cursor.execute(
                                "DELETE FROM event_views WHERE event_id = ?",
                                (event_id,),
                            )
                        else:
                            cursor.execute(
                                "DELETE FROM event_interests WHERE event_id = %s",
                                (event_id,),
                            )
                            cursor.execute(
                                "DELETE FROM event_views WHERE event_id = %s",
                                (event_id,),
                            )
                    ...
                    logger.info(
                        f"✅ Successfully cleaned up {len(expired_events)} expired events"
                    )
                    ...
                    logger.info(
                        f"📁 {archived_count} events in 32-day archive (from {archive_cutoff} to {current_date})"
                    )

                except Exception as cleanup_error:
                    cursor.execute("ROLLBACK")
                    logger.error(
                        f"❌ Event cleanup failed, transaction rolled back: {cleanup_error}"
                    )
                    raise
        ...
        try:
            ...
            self.scheduler.add_job(
                ...
                trigger=IntervalTrigger(
                    hours=6, start_date=datetime.utcnow() + timedelta(hours=2)
                ),
                id="event_refresh",
            ...
            self.scheduler.add_job(
                func=run_ai_sync,
                trigger=IntervalTrigger(
                    hours=6, start_date=datetime.utcnow() + timedelta(hours=4)
                ),
                id="ai_sync",
            ...
            self.scheduler.add_job(
                ...
                trigger=IntervalTrigger(
                    hours=6, start_date=datetime.utcnow() + timedelta(hours=3)
                ),
                id="event_cleanup",
            ...
            self.scheduler.add_job(
                ...
                trigger=IntervalTrigger(
                    hours=24, start_date=datetime.utcnow() + timedelta(hours=5)
                ),
                id="seo_population",
        ...
        for task_name, status in self.task_status.items():
            if status["last_run"]:
                ...
                if hours_since_last_run > 8:  # Alert if task hasn't run in 8+ hours
                    logger.warning(
                        f"⚠️ Task '{task_name}' hasn't run in {hours_since_last_run:.1f} hours"
                    )
...
class PasswordValidator:
    ...
    def validate_password(password: str) -> dict:
        ...
        validation_result = {"is_valid": True, "feedback": [], "strength": "weak"}
...
class EventBase(BaseModel):
    ...
    def validate_secondary_category(cls, v):
        ...
        if v not in categories:
            raise ValueError(
                f'Secondary category must be one of: {", ".join(categories)}'
            )
...
def verify_password(plain_password, hashed_password):
    ...
    try:
        ...
        # Use bytes slicing to handle multi-byte characters correctly
        ...
        # On error, try with direct bcrypt as fallback
        try:
            import bcrypt
            password_bytes = plain_password.encode('utf-8')[:72] if isinstance(plain_password, str) else plain_password[:72]
            return bcrypt.checkpw(password_bytes, hashed_password.encode('utf-8') if isinstance(hashed_password, str) else hashed_password)
        except Exception as e2:
            logger.error(f"Fallback password verification failed: {str(e2)}")
            return False
...
def get_password_hash(password):
    ...
    try:
        ...
        import bcrypt
        password_bytes = password.encode('utf-8')[:72] if isinstance(password, str) else password[:72]
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    try:
        with get_db() as conn:
            ...
            placeholder = get_placeholder()
            c.execute(
                f"SELECT id, email, role FROM users WHERE email = {placeholder}",
                (email,),
            )
            user = c.fetchone()
        ...
        if "timeout" in error_msg.lower() or "deadlock" in error_msg.lower():
            raise HTTPException(
                status_code=408, detail="Login request timed out. Please try again."
            )
        else:
            raise HTTPException(
                status_code=500,
    ...
    try:
        with get_db() as conn:
            ...
            try:
                ...
                if not user:
                    logger.warning(
                        f"Login failed for user: {form_data.username} - User not found"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            if user:  # Extra safety check
                try:
                    logger.info(
                        f"Generating access token for user: {form_data.username}"
                    )
    ...
    try:
        with get_db() as conn:
            ...
            try:
                ...
                logger.error(
                    f"Database health check failed during registration: {str(e)}"
                )
                raise HTTPException(
                    status_code=503,
                    detail="Database connection issues. Please try again later.",
            ...
            try:
                c.execute(
                    f"SELECT id FROM users WHERE email = {placeholder}", (user.email,)
                )
            ...
            try:
                ...
                # Enhanced logging for law enforcement compliance
                log_activity(
                    last_id, "registration", f"New user registered: {user.email}"
                )
...
async def request_password_reset(request: PasswordResetRequest):
    ...
    try:
        with get_db() as conn:
            ...
            c.execute(
                f"SELECT * FROM users WHERE email = {placeholder}", (request.email,)
            )
            user = c.fetchone()
            ...
            if not user:
                ...
                return {
                    "detail": "If your email is registered, you will receive a reset code"
                }
            ...
            c.execute(
                f"DELETE FROM password_resets WHERE email = {placeholder}",
                (request.email,),
            )
            ...
            try:
                ...
                email_sent = email_service.send_password_reset_email(
                    request.email, reset_code, user_name
                )
...
async def read_event(event_id: int):
    ...
    try:
        with get_db() as conn:
            ...
            c.execute(
                f"""SELECT id, title, description, short_description, date, start_time, end_time, end_date, 
    ...
    try:
        ...
        if event_id:
            ...
            if placeholder == "?":
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM events WHERE slug = %s AND id != %s",
                    (base_slug, event_id),
                )
        else:
            if placeholder == "?":
                # SQLite style
                cursor.execute(
                    "SELECT COUNT(*) FROM events WHERE slug = ?", (base_slug,)
                )
            else:
                # PostgreSQL style (use %s with psycopg2)
                cursor.execute(
                    "SELECT COUNT(*) FROM events WHERE slug = %s", (base_slug,)
                )
        ...
        logger.error(
            f"Error ensuring unique slug for '{base_slug}': {type(e).__name__}: {str(e)}"
        )
        ...
        fallback_slug = (
            f"{base_slug}-{timestamp_suffix}"
            if base_slug
            else f"event-{timestamp_suffix}"
        )
...
def get_actual_table_columns(cursor, table_name: str = "events") -> List[str]:
    ...
    try:
        if IS_PRODUCTION and DB_URL:
            ...
            try:
                ...
                if columns_info:
                    ...
                    logger.info(
                        f"✅ PostgreSQL schema detection: Found {len(columns)} columns via information_schema"
                    )
                    logger.debug(
                        f"Column details: {[(row[0], row[1], row[2]) for row in columns_info]}"
                    )
                    return columns
                else:
                ...
                logger.debug(
                    f"❌ information_schema query failed (fallback available): {type(e).__name__}"
                )
            ...
            try:
                ...
                if columns_info:
                    columns = [row[0] for row in columns_info]
                    logger.info(
                        f"✅ PostgreSQL schema detection: Found {len(columns)} columns via pg_class"
                    )
                    logger.debug(
                        f"System catalog columns: {[(row[0], row[1], row[2]) for row in columns_info]}"
                    )
                    return columns
                else:
                ...
                logger.debug(
                    f"❌ pg_class query failed (fallback available): {type(e).__name__}"
                )
            ...
            try:
                ...
                if cursor.description:
                    ...
                    logger.info(
                        f"✅ PostgreSQL schema detection: Found {len(columns)} columns via SELECT LIMIT 0"
                    )
                    return columns
                else:
            ...
            try:
                ...
                if columns_info:
                    ...
                    logger.info(
                        f"✅ SQLite schema detection: Found {len(columns)} columns"
                    )
                    return columns
                else:
                    logger.warning(
                        f"⚠️ PRAGMA table_info returned no results for {table_name}"
                    )
            except Exception as e:
        ...
        logger.error(
            f"❌ Critical error in get_actual_table_columns for {table_name}: {e}"
        )
    ...
    logger.warning(
        f"🔄 Using enhanced fallback columns ({len(fallback_columns)}) for {table_name}"
    )
...
async def create_event(
    ...
    try:
        ...
        logger.info(
            f"Auto-populated SEO fields: slug={event_data.get('slug')}, city={event_data.get('city')}, state={event_data.get('state')}, price={event_data.get('price')}"
        )
    except Exception as e:
    ...
    try:
        ...
        with get_db_transaction() as conn:
            ...
            try:
                ...
                if duplicate:
                    cursor.execute("ROLLBACK")
                    logger.warning(
                        f"Duplicate event prevented: {event_data['title']} by user {current_user['id']}"
                    )
                    raise HTTPException(
                        status_code=409,
                ...
                if (
                    "interest_count" not in filtered_event_data
                    and "interest_count" in actual_columns
                ):
                    filtered_event_data["interest_count"] = 0
                if (
                    "view_count" not in filtered_event_data
                    and "view_count" in actual_columns
                ):
                    filtered_event_data["view_count"] = 0

                # Build dynamic INSERT query
                columns = [
                    col for col in filtered_event_data.keys() if col in actual_columns
                ]
                ...
                logger.debug(
                    f"Dynamic INSERT with {len(columns)} columns: {columns[:5]}{'...' if len(columns) > 5 else ''}"
                )
                ...
                event_cache.clear()
                logger.info(
                    f"Successfully created event {event_id}: {event_data['title']} with SEO fields populated"
                )
        ...
        raise HTTPException(
            status_code=400, detail=f"Could not create event: {error_msg}"
        ) from None
...
async def update_event(
    event_id: int, event: EventCreate, current_user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        event_data = auto_populate_seo_fields(event_data)
        logger.info(
            f"Auto-populated SEO fields for update: slug={event_data.get('slug')}, city={event_data.get('city')}, state={event_data.get('state')}, price={event_data.get('price')}"
        )
    except Exception as e:
    ...
    try:
        with get_db() as conn:
            try:
                ...
                set_clause = ", ".join(
                    [f"{col} = {placeholder}" for col in update_columns]
                )
                query = f"UPDATE events SET {set_clause} WHERE id = {placeholder}"
                values = list(filtered_event_data.values()) + [event_id]

                logger.debug(
                    f"Dynamic UPDATE with {len(update_columns)} columns: {update_columns[:5]}{'...' if len(update_columns) > 5 else ''}"
                )
                ...
                if not updated_event:
                    cursor.execute("ROLLBACK")
                    raise HTTPException(
                        status_code=404, detail="Event not found after update"
                    )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Error updating event: {error_msg}"
                )
    ...
    try:
        with get_db() as conn:
            try:
                ...
                cursor.execute(
                    f"DELETE FROM events WHERE id = {placeholder}", (event_id,)
                )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Error deleting event: {error_msg}"
                )
...
async def delete_user(user_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        with get_db() as conn:
            ...
            c.execute(
                f"SELECT COUNT(*) as admin_count FROM users WHERE role = {placeholder}",
                (UserRole.ADMIN,),
            )
...
async def update_user_role(
    user_id: int, role: UserRole, current_user: dict = Depends(get_current_user)
):
    """
...
async def admin_reset_user_password(
    ...
    try:
        with get_db() as conn:
            ...
            # Update the user's password
            c.execute(
                f"UPDATE users SET hashed_password = {placeholder} WHERE id = {placeholder}",
...
async def get_activity_logs(
    current_user: dict = Depends(get_current_user), limit: int = 500, offset: int = 0
):
    """
...
def create_forensic_tables():
    ...
    try:
        with get_db() as conn:
            ...
            if IS_PRODUCTION and DB_URL:
                c.execute(
                    """CREATE TABLE IF NOT EXISTS media_forensic_data (
                    id SERIAL PRIMARY KEY,
                    user_id INTEGER,
                    event_id INTEGER,
                ...
                c.execute(
                    """CREATE TABLE IF NOT EXISTS user_forensic_data (
                    id SERIAL PRIMARY KEY,
            ...
            conn.commit()
            logger.info(
                "✅ Created forensic data tables for law enforcement compliance"
            )
...
async def get_automation_status():
    ...
    return {
        "automation": {
            ...
            "scheduler_running": (
                task_manager.scheduler.running if IS_PRODUCTION else False
            ),
            "last_sitemap_update": (
                task_manager.last_sitemap_update.isoformat()
                if task_manager.last_sitemap_update
                else None
            ),
            "last_event_refresh": (
                task_manager.last_event_refresh.isoformat()
                if task_manager.last_event_refresh
                else None
            ),
            ...
            "next_runs": (
                ...
                if IS_PRODUCTION and task_manager.scheduler.running
                else {}
            ),
        },
        "system": {
...
async def trigger_automated_task(task_name: str, background_tasks: BackgroundTasks):
    ...
    if not IS_PRODUCTION:
        raise HTTPException(
            status_code=400, detail="Automation only available in production"
        )
...
async def get_database_stats():
    ...
    try:
        with get_db() as conn:
            ...
            if IS_PRODUCTION and DB_URL:
                c.execute(
                    "SELECT COUNT(*) FROM events WHERE CAST(date AS DATE) >= CURRENT_DATE"
                )
            else:
            ...
            # Events with slugs
            c.execute(
                "SELECT COUNT(*) FROM events WHERE slug IS NOT NULL AND slug != ''"
            )
            ...
            if IS_PRODUCTION and DB_URL:
                c.execute(
                    "SELECT COUNT(*) FROM events WHERE CAST(date AS DATE) >= CURRENT_DATE AND slug IS NOT NULL AND slug != ''"
                )
            else:
                c.execute(
                    "SELECT COUNT(*) FROM events WHERE date >= date('now') AND slug IS NOT NULL AND slug != ''"
                )
            ...
            return {
                "database_type": (
                    "PostgreSQL" if (IS_PRODUCTION and DB_URL) else "SQLite"
                ),
                ...
                "expected_sitemap_urls": sitemap_eligible
                * 3,  # 3 URL formats per event
                "current_date": datetime.utcnow().strftime("%Y-%m-%d"),
...
async def debug_fix_event_images(
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                f"SELECT id, title, banner_image, logo_image FROM events WHERE id = {placeholder}",
                (event_id,),
            )
            event = cursor.fetchone()
            ...
            if updates:
                update_query = (
                    f"UPDATE events SET {', '.join(updates)} WHERE id = {placeholder}"
                )
                ...
                logger.info(
                    f"Fixed image data for event {event_id}: banner={banner_filename}, logo={logo_filename}"
                )
...
async def toggle_event_verification(
    ...
    try:
        with get_db() as conn:
            ...
            c.execute(
                f"UPDATE events SET verified = {placeholder} WHERE id = {placeholder}",
                (verified, event_id),
            )
            conn.commit()

            return {
                "detail": f"Event verification {'enabled' if verified else 'disabled'} successfully"
            }
    except HTTPException:
        raise
    except Exception as e:
...
async def track_event_view(
    event_id: int, user_id: int = None, browser_fingerprint: str = None
):
    """
    ...
    try:
        with get_db() as conn:
            ...
            if not cursor.fetchone():
                logger.warning(
                    f"Attempted to track view for non-existent event {event_id}"
                )
                return False
        ...
        logger.error(
            f"Error in track_event_view for event {event_id}: {type(e).__name__}: {str(e)}"
        )
        return False
...
async def toggle_event_interest(
    ...
    try:
        ...
        logger.info(
            f"🎯 Interest toggle - Event: {event_id}, User ID: {user_id}, Has Auth: {current_user is not None}"
        )
        ...
        with get_db() as conn:
            ...
            logger.info(
                f"✅ Interest {action} - Event: {event_id}, Count: {interest_count}, User: {user_id}, Fingerprint: {browser_fingerprint}"
            )
        ...
        logger.error(
            f"Error in interest toggle endpoint for event {event_id}: {type(e).__name__}: {str(e)}"
        )
...
async def track_event_view_endpoint(
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if not result:
                raise HTTPException(
                    status_code=404, detail="Event not found after view tracking"
                )
        ...
        logger.error(
            f"Error in view tracking endpoint for event {event_id}: {type(e).__name__}: {str(e)}"
        )
...
async def get_premium_status(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "events_remaining": (
            max(0, event_limit - current_month_events) if event_limit > 0 else None
        ),
...
def process_image(
    image_bytes: bytes, target_width: int, target_height: int, max_file_size_mb: int = 5
) -> bytes:
    """
    ...
    try:
        ...
        if width_diff <= 0.1 and height_diff <= 0.1:
            ...
            img.thumbnail(
                (target_width * 2, target_height * 2), Image.Resampling.LANCZOS
            )
        ...
        logger.info(
            f"Image processed: {len(image_bytes)} bytes -> {len(processed_bytes)} bytes, quality: {quality}"
        )
...
async def upload_event_banner(
    ...
    if not is_premium:
        raise HTTPException(
            status_code=403, detail="Premium subscription required for image uploads"
        )

    # Validate file type - only JPG and PNG
    if not file.content_type in ["image/jpeg", "image/jpg", "image/png"]:
        raise HTTPException(
            status_code=400, detail="File must be JPG or PNG format only"
        )
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                f"SELECT created_by FROM events WHERE id = {placeholder}", (event_id,)
            )
            event = cursor.fetchone()
            ...
            unique_filename = (
                f"banner_{event_id}_{uuid.uuid4().hex[:8]}_{file.filename}"
            )
            ...
            logger.info(
                f"Storing banner image in database for event {event_id} ({len(processed_image_bytes)} bytes as base64)"
            )
            ...
            if cursor.rowcount == 0:
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to link banner image to event"
                )
            ...
            log_activity(
                current_user["id"],
                "upload_banner",
                f"Uploaded banner image for event {event_id}",
            )
            log_media_activity(
                user_id=current_user["id"],
                event_id=event_id,
...
async def upload_event_logo(
    ...
    try:
        with get_db() as conn:
            ...
            logger.info(
                f"Storing logo image in database for event {event_id} ({len(processed_image_bytes)} bytes as base64)"
            )
            ...
            if cursor.rowcount == 0:
                logger.error(f"Failed to update event {event_id} - no rows affected")
                raise HTTPException(
                    status_code=500, detail="Failed to link logo image to event"
                )
            ...
            log_activity(
                current_user["id"],
                "upload_logo",
                f"Uploaded logo image for event {event_id}",
            )
            log_media_activity(
                user_id=current_user["id"],
                event_id=event_id,
...
async def delete_event_banner(
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                f"SELECT created_by, banner_image FROM events WHERE id = {placeholder}",
                (event_id,),
            )
            event = cursor.fetchone()
            ...
            if event["created_by"] != current_user["id"]:
                raise HTTPException(
                    status_code=403,
                    detail="You can only delete images from your own events",
                )
            ...
            log_activity(
                current_user["id"],
                "delete_banner",
                f"Deleted banner image for event {event_id}",
            )
...
async def delete_event_logo(
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                f"SELECT created_by, logo_image FROM events WHERE id = {placeholder}",
                (event_id,),
            )
            event = cursor.fetchone()
            ...
            log_activity(
                current_user["id"],
                "delete_logo",
                f"Deleted logo image for event {event_id}",
            )
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if not user_data or user_data["role"] not in ["premium", "admin"]:
                raise HTTPException(
                    status_code=404, detail="No active premium trial found"
                )
            ...
            try:
                ...
                if customers.data:
                    ...
                    if subscriptions.data:
                        raise HTTPException(
                            status_code=400,
                            detail="Cannot cancel trial - active subscription found. Use subscription cancellation instead.",
                        )
            ...
            log_activity(
                current_user["id"], "trial_cancelled", "Premium trial cancelled by user"
            )
            logger.info(
                f"✅ Premium trial cancelled for user {current_user['id']} ({current_user['email']})"
            )
            ...
            try:
                ...
                email_sent = email_service.send_trial_cancellation_email(
                    to_email=current_user["email"], user_name=user_name
                )

                if email_sent:
                    logger.info(
                        f"✅ Trial cancellation email sent to {current_user['email']}"
                    )
                else:
                    logger.error(
                        f"❌ Failed to send trial cancellation email to {current_user['email']}"
                    )
            except Exception as e:
...
async def convert_trial_to_subscription(
    ...
    try:
        ...
        with get_db() as conn:
            ...
            base_url = (
                "https://todo-events.com" if IS_PRODUCTION else "http://localhost:3000"
            )
            ...
            subscription_data = {
                "metadata": {
                    ...
                    "trial_expires_at": (
                        trial_expires_at.isoformat() if trial_expires_at else ""
                    ),
                }
            }
            ...
            logger.info(
                f"✅ Created {pricing_tier} trial conversion checkout session for user {current_user['id']} ({current_user['email']})"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to convert trial to subscription"
        )
        ...
        with get_db() as conn:
            ...
            if user_data["premium_expires_at"] and not has_stripe_subscription:
                ...
                trial_info = {
                    ...
                    "days_remaining": max(
                        0, (expires_at - datetime.now(timezone.utc)).days
                    ),
...
async def delete_user_account(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            if customers.data:
                ...
                for subscription in subscriptions.data:
                    ...
                    logger.info(
                        f"✅ Cancelled Stripe subscription {subscription.id} for account deletion"
                    )
            ...
            logger.warning(
                f"Stripe cancellation failed during account deletion: {str(e)}"
            )
        ...
        with get_db_transaction() as conn:
            ...
            cursor.execute(
                f"SELECT COUNT(*) as count FROM events WHERE created_by = {placeholder}",
                (user_id,),
            )
            deleted_items["events"] = cursor.fetchone()["count"]

            cursor.execute(
                f"SELECT COUNT(*) as count FROM event_interests WHERE user_id = {placeholder}",
                (user_id,),
            )
            deleted_items["interests"] = cursor.fetchone()["count"]

            cursor.execute(
                f"SELECT COUNT(*) as count FROM event_views WHERE user_id = {placeholder}",
                (user_id,),
            )
            deleted_items["views"] = cursor.fetchone()["count"]

            cursor.execute(
                f"SELECT COUNT(*) as count FROM page_visits WHERE user_id = {placeholder}",
                (user_id,),
            )
            ...
            cursor.execute(
                f"DELETE FROM event_interests WHERE event_id IN (SELECT id FROM events WHERE created_by = {placeholder})",
                (user_id,),
            )
            cursor.execute(
                f"DELETE FROM event_views WHERE event_id IN (SELECT id FROM events WHERE created_by = {placeholder})",
                (user_id,),
            )
            cursor.execute(
                f"DELETE FROM events WHERE created_by = {placeholder}", (user_id,)
            ...
            cursor.execute(
                f"DELETE FROM event_interests WHERE user_id = {placeholder}", (user_id,)
            )
            cursor.execute(
                f"DELETE FROM event_views WHERE user_id = {placeholder}", (user_id,)
            )
            cursor.execute(
                f"DELETE FROM page_visits WHERE user_id = {placeholder}", (user_id,)
            )

            # Delete any event reports made by this user
            cursor.execute(
                f"DELETE FROM event_reports WHERE reporter_email = {placeholder}",
                (user_email,),
            )
        ...
        log_activity(
            user_id, "account_deletion_request", f"User requested account deletion"
        )
        ...
        try:
            ...
            if email_sent:
                logger.info(
                    f"✅ Account deletion confirmation email sent to {user_email}"
                )
            else:
                logger.error(
                    f"❌ Failed to send account deletion confirmation email to {user_email}"
                )
        except Exception as e:
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if user_data["deletion_scheduled_at"]:
                scheduled_deletion = datetime.fromisoformat(
                    user_data["deletion_scheduled_at"].replace("Z", "+00:00")
                )
                if datetime.utcnow() >= scheduled_deletion:
                    raise HTTPException(
                        status_code=400, detail="Recovery period has expired"
                    )
        ...
        # Log the recovery
        log_activity(
            user_id, "account_deletion_cancelled", f"User cancelled account deletion"
        )
        ...
        try:
            ...
            email_sent = email_service.send_account_recovery_email(
                to_email=user_email, user_name=user_name
            )

            if email_sent:
                logger.info(
                    f"✅ Account recovery confirmation email sent to {user_email}"
                )
        except Exception as e:
        ...
        logger.error(
            f"Error cancelling account deletion for user {current_user['id']}: {str(e)}"
        )
...
async def create_checkout_session(
    ...
    try:
        ...
        base_url = (
            "https://todo-events.com" if IS_PRODUCTION else "http://localhost:5173"
        )
        ...
        if trial_ends_at:
            try:
                ...
                if trial_end_date > datetime.now(timezone.utc):
                    ...
                    trial_days = max(
                        1, (trial_end_date - datetime.now(timezone.utc)).days
                    )
                    subscription_data["trial_period_days"] = min(
                        trial_days, 30
                    )  # Stripe max is 30 days
                    logger.info(
                        f"🕒 Setting {trial_days} day trial for user {current_user['id']}"
                    )
                else:
                    logger.info(
                        f"⏰ Trial already expired for user {current_user['id']}, starting subscription immediately"
                    )
            except Exception as e:
        ...
        logger.info(
            f"✅ Created {pricing_tier} checkout session for user {current_user['id']} ({current_user['email']})"
        )

        # Ensure we have a checkout URL
        if not checkout_session.url:
            logger.error(
                f"Checkout session created but no URL returned: {checkout_session.id}"
            )
            raise HTTPException(
                status_code=500, detail="No checkout URL received from Stripe"
            )
...
async def cancel_subscription(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.info(
            f"✅ Marked subscription {subscription.id} for cancellation for user {current_user['id']} ({current_user['email']})"
        )
        ...
        try:
            ...
            if (
                upcoming
                and hasattr(upcoming, "lines")
                and upcoming.lines
                and upcoming.lines.data
            ):
                for line in upcoming.lines.data:
                    if (
                        hasattr(line, "period")
                        and line.period
                        and hasattr(line.period, "start")
            ...
            try:
                cancels_at_ts = getattr(
                    canceled_subscription, "current_period_end", None
                )
                if cancels_at_ts:
                    access_until = datetime.fromtimestamp(cancels_at_ts).isoformat()
                    logger.info(
                        f"🗓️ Using subscription current_period_end: {access_until}"
                    )
        ...
        try:
            ...
            if email_sent:
                logger.info(
                    f"✅ Cancellation confirmation email sent to {current_user['email']}"
                )
            else:
                logger.error(
                    f"❌ Failed to send cancellation confirmation email to {current_user['email']}"
                )
        except Exception as e:
...
async def cancel_subscription_immediately(
    ...
    try:
        ...
        logger.info(
            f"✅ Immediately canceled subscription {subscription.id} for user {current_user['id']} ({current_user['email']})"
        )
        ...
        try:
            ...
            if email_sent:
                logger.info(
                    f"✅ Immediate cancellation email sent to {current_user['email']}"
                )
            else:
                logger.error(
                    f"❌ Failed to send immediate cancellation email to {current_user['email']}"
                )
        except Exception as e:
...
async def reactivate_subscription(
    ...
    try:
        ...
        logger.info(
            f"✅ Reactivated subscription {subscription_id} for user {current_user['id']} ({current_user['email']})"
        )
        ...
        try:
            ...
            try:
                upcoming = stripe.Invoice.create_preview(
                    customer=customer.id, subscription=subscription_id
                )
                if (
                    upcoming
                    and hasattr(upcoming, "lines")
...
async def get_detailed_subscription_status(
    ...
    try:
        ...
        for sub in subscriptions.data:
            ...
            logger.info(
                f"Available subscription attributes: {list(sub.keys()) if hasattr(sub, 'keys') else 'No keys method'}"
            )
            ...
            try:
                ...
                if current_period_start_ts:
                    actual_period_start = datetime.fromtimestamp(
                        current_period_start_ts
                    ).isoformat()
                    logger.info(
                        f"🕒 Actual current period start: {actual_period_start} (ts: {current_period_start_ts})"
                    )
            ...
            try:
                ...
                if current_period_end_ts:
                    actual_period_end = datetime.fromtimestamp(
                        current_period_end_ts
                    ).isoformat()
                    logger.info(
                        f"🕒 Actual current period end: {actual_period_end} (ts: {current_period_end_ts})"
                    )
            except (ValueError, TypeError, OSError, AttributeError):
                pass
            ...
            try:
                logger.info(
                    f"Attempting to fetch upcoming invoice preview for customer: {customer.id}"
                )
                upcoming = stripe.Invoice.create_preview(
                        ...
                        for line in upcoming.lines.data:
                            if hasattr(line, "period") and line.period:
                                if hasattr(line.period, "start") and line.period.start:
                                    ...
                                    logger.info(
                                        f"📅 Next billing date from upcoming invoice: {next_billing_date}"
                                    )
                                break
                ...
                logger.warning(
                    f"⚠️ Could not fetch upcoming invoice preview: {upcoming_error}"
                )
            ...
            if not actual_period_end:
                ...
                try:
                    ...
                    if latest_invoice_id:
                        logger.info(
                            f"Fetching latest invoice as fallback: {latest_invoice_id}"
                        )
                        ...
                        if invoice:
                            if hasattr(invoice, "period_end") and invoice.period_end:
                                ...
                                logger.info(
                                    f"Got actual period end from latest invoice: {actual_period_end}"
                                )
                except Exception as invoice_error:
                    logger.warning(
                        f"Could not fetch latest invoice data: {invoice_error}"
                    )

            try:
                canceled_at_ts = getattr(sub, "canceled_at", None)
                canceled_at = (
                    datetime.fromtimestamp(canceled_at_ts).isoformat()
                    if canceled_at_ts
                    else None
                )
            except (ValueError, TypeError, OSError, AttributeError):
            ...
            if cancel_at_period_end:
                ...
                logger.info(
                    f"🗓️ Subscription scheduled for cancellation, access until: {actual_period_end}"
                )
            ...
            try:
                if hasattr(sub, "items") and sub.items and sub.items.data:
                    if hasattr(sub.items.data[0], "price") and sub.items.data[0].price:
                        ...
                        currency = (
                            getattr(sub.items.data[0].price, "currency", "usd") or "usd"
                        )
            ...
            logger.info(
                f"Actual period: start={actual_period_start}, end={actual_period_end}"
            )
...
async def quick_upgrade_user_3():
    ...
    try:
        with get_db() as conn:
            ...
            cursor.execute(
                f"SELECT id, email, role FROM users WHERE id = {placeholder}", (3,)
            )
            ...
            logger.info(
                f"✅ Manually upgraded user 3 ({user['email']}) to premium due to webhook processing issues"
            )
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        logger.info(
            f"🔔 Webhook payload size: {len(payload)}, signature present: {bool(sig_header)}"
        )
        ...
        logger.info(
            f"🔔 Attempting to construct Stripe event with secret: {STRIPE_WEBHOOK_SECRET[:8]}..."
        )
        ...
        logger.info(
            f"🔔 SUCCESS! Received Stripe webhook: {event['type']} (ID: {event.get('id', 'unknown')})"
        )
        ...
        if event["type"] == "checkout.session.completed":
            ...
            logger.info(
                f"🔔 Processing invoice.payment_succeeded for invoice: {invoice.get('id')}"
            )
            ...
            subscription = event["data"]["object"]
            logger.info(
                f"🔔 Processing customer.subscription.paused for subscription: {subscription.get('id')}"
            )
            ...
            subscription = event["data"]["object"]
            logger.info(
                f"🔔 Processing customer.subscription.resumed for subscription: {subscription.get('id')}"
            )
            await handle_subscription_resumed(subscription)
        elif event["type"] == "invoice.created":
            invoice = event["data"]["object"]
            logger.info(
                f"🔔 Processing invoice.created for invoice: {invoice.get('id')}"
            )
            await handle_invoice_created(invoice)
        elif event["type"] == "invoice.upcoming":
            invoice = event["data"]["object"]
            logger.info(
                f"🔔 Processing invoice.upcoming for invoice: {invoice.get('id')}"
            )
...
async def handle_successful_payment(session):
    ...
    try:
        logger.info(
            f"🔔 Processing successful payment for session: {session.get('id')}"
        )
        ...
        logger.info(
            f"🔔 Processing payment for user {user_id} ({user_email}) - pricing tier: {pricing_tier}"
        )
        ...
        with get_db_transaction() as conn:
            ...
            cursor.execute(
                f"SELECT id, email, role FROM users WHERE id = {placeholder}",
                (user_id,),
            )
            user = cursor.fetchone()
            ...
            logger.info(
                f"🔔 Found user: {user['email']}, current role: {user['role']}, upgrading to: {new_role}"
            )
            ...
            log_activity(
                ...
                f"{new_role.title()} subscription activated via Stripe for {user_email}",
            )

            logger.info(
                f"✅ {new_role.title()} activated for user {user_id} ({user_email}) via Stripe payment, expires: {expires_at}"
            )
            ...
            try:
                ...
                if new_role == "enterprise":
                    ...
                    if email_sent:
                        logger.info(
                            f"✅ Enterprise notification email sent to {user_email}"
                        )
                    else:
                        logger.error(
                            f"❌ Failed to send enterprise notification email to {user_email}"
                        )
                else:
                ...
                if email_sent:
                    ...
                    logger.error(
                        f"❌ Failed to send premium notification email to {user_email}"
                    )
            except Exception as e:
                logger.error(
                    f"❌ Error sending {new_role} notification email: {str(e)}"
                )
...
async def handle_subscription_created(subscription):
    ...
    try:
        ...
        logger.info(
            f"📝 Subscription created for user {user_id} ({user_email}): {subscription['id']}"
        )
...
async def handle_subscription_renewal(invoice):
    ...
    try:
        ...
        with get_db_transaction() as conn:
            ...
            cursor.execute(
                f"SELECT premium_expires_at, role FROM users WHERE id = {placeholder}",
                (user_id,),
            )
            user_data = cursor.fetchone()
            ...
            if user_data and user_data["premium_expires_at"]:
                ...
                current_expires = (
                    datetime.fromisoformat(
                        user_data["premium_expires_at"].replace("Z", "+00:00")
                    )
                    if isinstance(user_data["premium_expires_at"], str)
                    else user_data["premium_expires_at"]
                )
                # If current expiration is in the past, start from now
                base_date = (
                    max(current_expires, datetime.utcnow())
                    if current_expires
            ...
            subscription_type = (
                "Enterprise" if pricing_tier == "enterprise" else "Premium"
            )
            log_activity(
                user_id,
                "stripe_renewal",
                f"{subscription_type} subscription renewed via Stripe",
            )
...
async def handle_subscription_cancelled(subscription):
    ...
    try:
        ...
        with get_db_transaction() as conn:
            ...
            if not user_email:
                cursor.execute(
                    f"SELECT email FROM users WHERE id = {placeholder}", (user_id,)
                )
                user_data = cursor.fetchone()
                if user_data:
            ...
            log_activity(
                user_id,
                "stripe_cancellation",
                f"Premium subscription cancelled via Stripe",
            )
            ...
            if user_email:
                try:
                    ...
                    if email_sent:
                        logger.info(
                            f"✅ Webhook cancellation email sent to {user_email}"
                        )
                    else:
                        logger.error(
                            f"❌ Failed to send webhook cancellation email to {user_email}"
                        )
                except Exception as e:
                    logger.error(
                        f"❌ Error sending webhook cancellation email: {str(e)}"
                    )
    ...
    try:
        ...
        with get_db_transaction() as conn:
            ...
            log_activity(
                ...
                f"Premium subscription paused via Stripe",
            )
...
async def handle_subscription_updated(subscription):
    ...
    try:
        ...
        if "metadata" in subscription and subscription["metadata"].get("user_id"):
            ...
            if customer_id:
                with get_db() as conn:
                    ...
                    placeholder = get_placeholder()
                    cursor.execute(
                        f"SELECT id, email FROM users WHERE stripe_customer_id = {placeholder}",
                        (customer_id,),
                    )
                    user_data = cursor.fetchone()
                    if user_data:
        ...
        if not user_id:
            logger.warning(
                f"Could not find user for subscription update: {subscription.get('id')}"
            )
            return
        ...
        with get_db_transaction() as conn:
            ...
            if subscription_status == "active":
                cursor.execute(
                    f"SELECT role FROM users WHERE id = {placeholder}", (user_id,)
                )
                user_data = cursor.fetchone()
                ...
                if user_data and user_data["role"] != "premium":
                    ...
                    logger.info(
                        f"✅ Updated user {user_id} to premium due to subscription update"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ Sent subscription update email to {user_email}"
                        )
                    except Exception as e:
                        logger.error(
                            f"❌ Failed to send subscription update email: {str(e)}"
                        )
            ...
            log_activity(
                ...
                f"Subscription {subscription_id} updated - status: {subscription_status}",
            )
            logger.info(
                f"📝 Subscription updated for user {user_id}: {subscription_id} - {subscription_status}"
            )
...
async def handle_subscription_resumed(subscription):
    ...
    try:
        ...
        if not user_id:
            logger.warning(
                f"Could not find user for subscription resume: {subscription.get('id')}"
            )
            return
        ...
        with get_db_transaction() as conn:
            ...
            log_activity(
                ...
                f"Premium subscription resumed via Stripe",
            )
            ...
            try:
                ...
                email_service.send_email(
                    user_email, "TodoEvents Premium - Welcome Back!", email_content
                )
...
async def handle_invoice_created(invoice):
    ...
    try:
        ...
        log_activity(
            ...
            f"Invoice {invoice_id} created - amount: {amount_due/100:.2f} {currency.upper()}",
        )
        ...
        try:
            formatted_amount = (
                ...
                else f"{amount_due/100:.2f} {currency.upper()}"
            )
            ...
            logger.info(
                f"✅ Sent invoice notification email to {user_email} for {formatted_amount}"
            )
        ...
        logger.info(
            f"📄 Invoice created for user {user_id}: {invoice_id} - {formatted_amount}"
        )
...
async def handle_invoice_upcoming(invoice):
    ...
    try:
        ...
        log_activity(
            ...
            f"Upcoming invoice {invoice_id} - amount: {amount_due/100:.2f} {currency.upper()}",
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Sent upcoming payment notification email to {user_email} for {formatted_amount}"
            )

        except Exception as e:
            logger.error(
                f"❌ Failed to send upcoming payment notification email: {str(e)}"
            )

        logger.info(
            f"📅 Upcoming invoice processed for user {user_id}: {invoice_id} - {formatted_amount}"
        )
...
async def get_user_analytics(
    ...
    try:
        with get_db() as conn:
            ...
            total_interests = sum(
                event.get("interest_count", 0) or 0 for event in event_list
            )
            ...
            try:
                ...
                existing_tables = (
                    [row[0] for row in table_results] if table_results else []
                )
                ...
                if "event_views" in existing_tables:
                    ...
                    try:
                        ...
                        if start_date:
                            query += f" AND ev.created_at >= {placeholder}"
                            params.append(start_date)
                        if end_date:
                            query += f" AND ev.created_at <= {placeholder}"
                            params.append(end_date)

                        query += " GROUP BY DATE(ev.created_at) ORDER BY date"
                ...
                if "event_interests" in existing_tables:
                    ...
                    try:
                        ...
                        if start_date:
                            query += f" AND ei.created_at >= {placeholder}"
                            params.append(start_date)
                        if end_date:
                            query += f" AND ei.created_at <= {placeholder}"
                            params.append(end_date)

                        query += " GROUP BY DATE(ei.created_at) ORDER BY date"

                        cursor.execute(query, tuple(params))
                        interest_results = cursor.fetchall()
                    except Exception as interest_error:
                        logger.warning(
                            f"Interest trends query failed: {str(interest_error)}"
                        )
                        interest_results = []
                    interest_trends = []
            ...
            engagement_rate = (
                round((total_interests / total_views * 100), 2)
                if total_views > 0
                else 0
            )
            avg_interests = (
                round(total_interests / total_events) if total_events > 0 else 0
            )
            ...
            for cat, stats in category_stats.items():
                engagement = (
                    round((stats["interests"] / stats["views"] * 100), 2)
                    if stats["views"] > 0
                    else 0
                )
            ...
            for event in event_list:
                ...
                location = (
                    ...
                    else city if city != "Unknown" else "Unknown Location"
                )

                if location not in geographic_stats:
                    geographic_stats[location] = {
                        "events": 0,
                        "views": 0,
                        "interests": 0,
                    }
                ...
                geographic_stats[location]["interests"] += (
                    event.get("interest_count", 0) or 0
                )
            ...
            # Determine the date range for the time series
            if start_date and end_date:
                start = datetime.strptime(start_date, "%Y-%m-%d")
                end = datetime.strptime(end_date, "%Y-%m-%d")
            else:
                # If no date range, use the min/max dates from event_list
                all_dates = [
                    datetime.strptime(
                        event.get("created_at", "").split("T")[0], "%Y-%m-%d"
                    )
                    for event in event_list
                ...
                if all_dates:
                    ...
                    start = datetime.utcnow() - timedelta(
                        days=30
                    )  # Default to last 30 days if no events
                    end = datetime.utcnow()
            ...
            response_data = {
                ...
                "top_performing_events": sorted(
                    event_list, key=lambda x: x.get("view_count", 0), reverse=True
                )[:10],
            ...
            if export_csv:
                ...
                if export_csv == "events":
                    ...
                    for event in response_data.get("all_events", []):
                        ...
                        engagement = (
                            round((interests / views * 100), 2) if views > 0 else 0
                        )
...
async def get_event_analytics(
    ...
    logger.info(
        f"🔍 Getting analytics for event {event_id} by user {current_user.get('id')}"
    )
    ...
    try:
        with get_db() as conn:
            ...
            if (
                ...
                raise HTTPException(
                    status_code=403,
                    detail="Not authorized to view this event's analytics",
                )
            ...
            for i in range(30):
                ...
                days_since_creation = (
                    current_date - timedelta(days=29 - i) - created_date
                ).days
                if days_since_creation >= 0:
                    # Simulate decreasing view rate over time
                    views_multiplier = max(0.1, 1 - (days_since_creation * 0.05))
                    daily_views = max(
                        0, int(total_views * views_multiplier * 0.1)
                    )  # 10% of total distributed
                    daily_interests = max(
                        0, int(total_interests * views_multiplier * 0.1)
                    )
                else:
                    daily_views = 0
                    daily_interests = 0

                time_series[date] = {"views": daily_views, "interests": daily_interests}
            ...
            if city and state:
                ...
                try:
                    ...
                    if (
                        ...
                        city_slug = (
                            slug_parts[2].replace("-", " ").title()
                        )  # daytona-beach -> Daytona Beach
                        location = f"{city_slug}, {state_code}, {country}"
                        logger.info(
                            f"📍 Extracted location from slug: '{slug}' -> '{location}'"
                        )
                except Exception as e:
            ...
            geographic_data = {
                location: {"views": total_views, "interests": total_interests}
            }
            ...
            logger.info(
                f"🔍 Event analytics for {event_id}: title='{event['title']}', category='{event['category']}', location='{location}', slug='{event.get('slug', 'None')}'"
            )
            ...
            response_data = {
                ...
                "performance_metrics": {
                    "view_velocity": round(
                        total_views / max(1, (current_date - created_date).days), 2
                    ),
...
async def list_user_events_debug(current_user: dict = Depends(get_current_user)):
    ...
    logger.info(
        f"🔍 /events/manage-debug called by user {current_user.get('id', 'unknown')}"
    )
    ...
    try:
        with get_db() as conn:
            ...
            # Simple query
            c.execute(
                f"SELECT * FROM events WHERE created_by = {placeholder} ORDER BY date, start_time LIMIT 5",
                (current_user["id"],),
            )
            events = c.fetchall()
...
async def get_user_events(current_user: dict = Depends(get_current_user)):
    ...
    logger.info(
        f"🔍 /user/events called by user {current_user.get('id', 'unknown')} ({current_user.get('email', 'unknown')})"
    )
            ...
            for event in events:
                try:
                    ...
                    event_dict["description"] = (
                        event_dict.get("description") or "No description"
                    )
                    event_dict["address"] = (
                        event_dict.get("address") or "Address not provided"
                    )
                    ...
                    event_dict["is_published"] = bool(
                        event_dict.get("is_published", True)
                    )
                    ...
                    logger.error(
                        f"🔍 Error processing event {event.get('id', 'unknown')}: {event_error}"
                    )
                    continue
...
async def list_user_events_no_validation(
    ...
    try:
        with get_db() as conn:
            ...
            query = f"SELECT * FROM events WHERE created_by = {placeholder} ORDER BY date, start_time"
            logger.info(
                f"🔍 Executing query: {query} with user_id={current_user['id']}"
            )
            c.execute(query, (current_user["id"],))
            events = c.fetchall()
            ...
            for i, event in enumerate(events):
                try:
                    ...
                    logger.info(
                        f"🔍 Processing event {i+1}/{len(events)}: ID={event_id}"
                    )
                    ...
                    try:
                        event_dict = convert_event_datetime_fields(event_dict)
                        logger.info(
                            f"🔍 Event {event_id}: Successfully processed and converted"
                        )
                    except Exception as conversion_error:
                        logger.error(
                            f"🔍 Event {event_id}: Datetime conversion failed: {conversion_error}"
                        )
            ...
            logger.info(
                f"🔍 Successfully processed {len(event_list)} events, returning response"
            )
            return event_list
...
async def list_user_events_robust(current_user: dict = Depends(get_current_user)):
    ...
    """
    logger.info(
        f"🔍 /events/manage-fix called by user {current_user.get('id', 'unknown')} ({current_user.get('email', 'unknown')})"
    )
        ...
        with get_db() as conn:
            ...
            for i, event in enumerate(events):
                try:
                    ...
                    event_dict["title"] = (
                        str(event_dict.get("title", "")).strip() or "Untitled Event"
                    )
                    event_dict["description"] = (
                        str(event_dict.get("description", "")).strip()
                        or "No description provided"
                    )
                    event_dict["date"] = (
                        str(event_dict.get("date", "")).strip() or "2024-01-01"
                    )
                    event_dict["start_time"] = (
                        str(event_dict.get("start_time", "")).strip() or "00:00"
                    )
                    event_dict["category"] = (
                        str(event_dict.get("category", "")).strip() or "other"
                    )
                    event_dict["address"] = (
                        str(event_dict.get("address", "")).strip()
                    ...
                    try:
                        ...
                        logger.warning(
                            f"🔍 Event {event_id}: Invalid lat value, using default 0.0"
                        )
                        event_dict["lat"] = 0.0
                    try:
                        ...
                        logger.warning(
                            f"🔍 Event {event_id}: Invalid lng value, using default 0.0"
                        )
                        event_dict["lng"] = 0.0

                    # Required integer fields
                    event_dict["created_by"] = int(
                        event_dict.get("created_by", current_user["id"])
                    )
                    event_dict["id"] = int(event_dict.get("id", 0))

                    # Required datetime field
                    event_dict["created_at"] = str(
                    ...
                    event_dict["view_count"] = int(event_dict.get("view_count", 0) or 0)
                    event_dict["secondary_category"] = event_dict.get(
                        "secondary_category"
                    )
                    ...
                    event_dict["short_description"] = event_dict.get(
                        "short_description"
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"🔍 Event {event_id}: Invalid price value, using default 0.0"
                        )
...
class RouteEventRequest(BaseModel):
    ...
    dateRange: Optional[Dict[str, str]] = (
        None  # Optional {startDate: str, endDate: str}
    )
    ...
    try:
        with get_db() as conn:
            ...
            try:
                ...
                logger.info(
                    f"Premium columns check: {existing_columns}, has_premium_columns: {has_premium_columns}"
                )
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(
            status_code=500, detail=f"Error retrieving premium users: {str(e)}"
        )
    ...
    try:
        with get_db() as conn:
            ...
            log_activity(
                current_user["id"],
                "grant_premium",
                f"Granted {request.months} months premium to {user['email']}",
            )
...
async def remove_premium(user_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        with get_db() as conn:
            ...
            log_activity(
                current_user["id"],
                "remove_premium",
                f"Removed premium from {user['email']}",
            )

            return {"detail": "Premium access removed", "user_email": user["email"]}
    except HTTPException:
        raise
    except Exception as e:
    ...
    try:
        with get_db() as conn:
            ...
            if existing_user:
                ...
                try:
                    if IS_PRODUCTION and DB_URL:
                        ...
                        existing_columns = [
                            col[1]
                            for col in columns_info
                            if col[1] in ["premium_expires_at", "premium_granted_by"]
                        ]
                ...
                log_activity(
                    current_user["id"],
                    "premium_invite_existing",
                    f"Upgraded existing user {request.email} to {request.months} months premium",
                )
                ...
                try:
                    ...
                    if email_sent:
                        logger.info(
                            f"✅ Premium upgrade notification email sent to {request.email}"
                        )
                    else:
                        logger.error(
                            f"❌ Failed to send premium upgrade notification email to {request.email}"
                        )
                except Exception as e:
                    logger.error(
                        f"❌ Error sending premium upgrade notification email: {str(e)}"
                    )
...
async def invite_enterprise_user(
    request: EnterpriseInviteRequest, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        with get_db() as conn:
            ...
            if existing_user:
                ...
                log_activity(
                    current_user["id"],
                    "enterprise_invite_existing",
                    f"Upgraded existing user {request.email} to {request.months} months enterprise",
                )
                ...
                try:
                    ...
                    if email_sent:
                        logger.info(
                            f"✅ Enterprise upgrade notification email sent to {request.email}"
                        )
                    else:
                        logger.error(
                            f"❌ Failed to send enterprise upgrade notification email to {request.email}"
                        )
                except Exception as e:
                    logger.error(
                        f"❌ Error sending enterprise upgrade notification email: {str(e)}"
                    )
    ...
    try:
        with get_db() as conn:
            ...
            completion_rate = round(
                (result[1] / result[0] * 100) if result[0] > 0 else 0, 1
            )
...
async def export_enterprise_data(current_user: dict = Depends(get_current_user)):
...
async def get_enterprise_overview(current_user: dict = Depends(get_current_user)):
    ...
    try:
        with get_db() as conn:
            ...
            logger.info(
                f"Enterprise overview request from user {current_user['id']} with role {current_user['role']}"
            )
            ...
            if current_user["role"] == UserRole.ENTERPRISE:
                ...
                logger.info(
                    f"Executing query: {query} with params: {(current_user['id'],)}"
                )
                c.execute(query, (current_user["id"],))
                result = c.fetchone()
                ...
                try:
                    ...
                    c.execute(
                        f"SELECT COUNT(*) as this_month, 0 as last_month FROM events WHERE created_by = {placeholder}",
                        (current_user["id"],),
                    )
            else:
                # Admin sees all data
                ...
                try:
                    ...
                    c.execute(
                        "SELECT COUNT(*) as this_month, 0 as last_month FROM events"
                    )
            ...
            if isinstance(growth_data, dict) or hasattr(growth_data, "get"):
                ...
                this_month = (
                    growth_data[0] if growth_data and len(growth_data) > 0 else 0
                )
                last_month = (
                    growth_data[1] if growth_data and len(growth_data) > 1 else 0
                )

            event_growth = (
                ((this_month - last_month) / max(last_month, 1)) * 100
                if last_month > 0
                else 0
            )
            ...
            if isinstance(client_data, dict) or hasattr(client_data, "get"):
                ...
                total_clients = (
                    client_data[0] if client_data and len(client_data) > 0 else 0
                )
                active_clients = (
                    client_data[1] if client_data and len(client_data) > 1 else 0
                )
            ...
            return {
                ...
                "client_engagement_rate": round(
                    (active_clients / max(total_clients, 1)) * 100, 1
                ),
            }
...
async def get_enterprise_clients(current_user: dict = Depends(get_current_user)):
        ...
        with get_db() as conn:
            ...
            if search:
                where_conditions.append(
                    f"(e.title {db_parts['like_op']} {placeholder} OR e.description {db_parts['like_op']} {placeholder})"
                )
            ...
            for row in c.fetchall():
                ...
                event = {
                    ...
                    "title": row[1],
                    "description": (
                        row[2][:100] + "..." if row[2] and len(row[2]) > 100 else row[2]
                    ),
                    "date": row[3],
                    "start_time": row[4],
                    "end_time": row[5],
                    ...
                    "is_free": not bool(
                        row[8] and row[8].strip()
                    ),  # fee_required empty means free
    ...
    try:
        with get_db() as conn:
            ...
            if user["role"] not in ["premium", "enterprise"]:
                raise HTTPException(
                    status_code=400,
                    detail="User does not have premium or enterprise access",
                )
            ...
            log_activity(
                ...
                f"Sent {user['role']} notification to {user['email']}",
            )
            ...
            try:
                ...
                if user.get("premium_granted_by"):
                    c.execute(
                        f"SELECT email FROM users WHERE id = {placeholder}",
                        (user["premium_granted_by"],),
                    )
                ...
                if user["role"] == "enterprise":
                    ...
                    if email_sent:
                        logger.info(
                            f"✅ Enterprise notification email sent to {user['email']}"
                        )
                    else:
                        logger.error(
                            f"❌ Failed to send enterprise notification email to {user['email']}"
                        )
                else:
                    # Send premium notification email
                    email_sent = email_service.send_premium_notification_email(
                ...
                if email_sent:
                    logger.info(
                        f"✅ Premium notification email sent to {user['email']}"
                    )
                else:
                    logger.error(
                        f"❌ Failed to send premium notification email to {user['email']}"
                    )
            except Exception as e:
                logger.error(
                    f"❌ Error sending {user['role']} notification email: {str(e)}"
                )
...
async def create_referral_link(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error creating referral link: {str(e)}"
        )
    finally:
        cursor.close()
...
async def get_referral_links(request: Request):
    ...
    try:
            ...
            links.append(
                {
                    ...
                    "stats": {
                        ...
                        "commission_per_click": round(
                            float(row[10]) / max(row[7], 1), 2
                        ),
                    },
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching referral links: {str(e)}"
        )
    finally:
        cursor.close()
...
async def toggle_referral_link(request: Request, link_id: int):
    ...
    try:
        ...
        cursor.connection.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error toggling referral link: {str(e)}"
        )
    finally:
        cursor.close()
...
async def get_referral_analytics(request: Request, days: int = 30):
    ...
    try:
        ...
        return {
            ...
            "overview": {
                ...
                "conversion_rate": round(
                    ((overall[1] or 0) / max(overall[0] or 1, 1)) * 100, 2
                ),
                "avg_commission_per_conversion": round(
                    float(overall[3] or 0) / max(overall[1] or 1, 1), 2
                ),
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching referral analytics: {str(e)}"
        )
    finally:
        cursor.close()
...
async def create_referral_commission(
    ...
    try:
        ...
        cursor.execute(
            "SELECT commission_rate FROM referral_links WHERE id = ?",
            (referral_link_id,),
        )
        link = cursor.fetchone()
        ...
        cursor.connection.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error creating commission: {str(e)}"
        )
    finally:
        cursor.close()
...
class SimpleCache:
    ...
    def clear_pattern(self, pattern: str) -> int:
        ...
        with self._lock:
            keys_to_delete = [
                key for key in self.cache.keys() if key.startswith(pattern)
            ]
...
async def bulk_create_events_simple(
    bulk_events: BulkEventCreate, current_user: dict = Depends(get_current_user)
):
    """
    ...
    logger.info(
        f"Admin {current_user['email']} initiating SIMPLE bulk event creation for {len(bulk_events.events)} events"
    )
    ...
    for i, event_data in enumerate(bulk_events.events):
        try:
            ...
            success_count += 1
            logger.info(
                f"✅ Successfully created event {i+1}: '{event_data.title}' (ID: {response_event.id})"
            )
            ...
            error_msg = str(e)
            logger.error(
                f"❌ Error creating event {i+1} ({event_data.title}): {error_msg}"
            )
            errors.append(
                {
                    "event_index": i + 1,
    ...
    logger.info(
        f"✅ Simple bulk event creation completed. Success: {success_count}, Errors: {error_count}"
    )
...
async def bulk_create_events(
    ...
    logger.info(
        f"Admin {current_user['email']} initiating ROBUST bulk event creation for {len(bulk_events.events)} events using proven create_event logic"
    )
    ...
    logger.info(
        f"✅ Robust bulk event creation completed. Success: {success_count}, Errors: {error_count}"
    )
    ...
    try:
        ...
        if result is None:
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Could not extract count from indexable result: {result}"
                )
                count = 0
        else:
            try:
        ...
        if count > 0:
            ...
            logger.info(
                f"✅ Slug '{base_slug}' exists ({count} times), generated unique: '{unique_slug}'"
            )
            return unique_slug
        else:
        ...
        logger.error(
            f"❌ Slug uniqueness check failed for '{base_slug}': {type(e).__name__}: {e}"
        )
        ...
        with get_db() as conn:
            ...
            for column_name, column_spec in required_columns.items():
                if column_name not in current_columns:
                    try:
                        sql = (
                            f"ALTER TABLE events ADD COLUMN {column_name} {column_spec}"
                        )
...
def convert_event_datetime_fields(event_dict):
    ...
    for field in datetime_fields:
        if field in event_dict and isinstance(event_dict[field], datetime):
            ...
            field in event_dict
            and event_dict[field] is not None
            and not isinstance(event_dict[field], str)
        ):
...
async def get_events_by_location(
    state: str, city: str, limit: int = 100, offset: int = 0
):
    ...
    try:
        with get_db() as conn:
            ...
            share_card_data = {
                ...
                "title": event_dict["title"],
                "date": event_dict["date"],
                "time": f"{event_dict['start_time']}"
                + (
                    f" - {event_dict['end_time']}" if event_dict.get("end_time") else ""
                ),
                ...
                "description": (
                    event_dict["description"][:120] + "..."
                    if len(event_dict["description"]) > 120
                    else event_dict["description"]
                ),
...
async def get_event_share_card_png(event_id: int):
    ...
    try:
        ...
        with get_db() as conn:
            ...
            event_title = (
                event_row["title"][:30]
                if len(event_row["title"]) > 30
                else event_row["title"]
            )
        ...
        return RedirectResponse(
            url="https://via.placeholder.com/800x600/3B82F6/FFFFFF?text=TodoEvents",
            status_code=302,
        )
...
async def populate_production_seo_fields():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"SEO population failed: {str(e)}")
    ...
    try:
        with get_db() as conn:
            ...
            null_count = (
                result[0]
                if isinstance(result, (tuple, list))
                else result.get("COUNT(*)", 0) if result else 0
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fixing NULL end_time values: {str(e)}"
        )
...
async def get_analytics_metrics(
    ...
    try:
        with get_db() as conn:
            ...
            # Parse filters to match time-series endpoint
            excluded_user_ids = []
            if exclude_users:
                try:
                    excluded_user_ids = [
                        int(uid.strip())
                        for uid in exclude_users.split(",")
                        if uid.strip()
                    ]
                except ValueError:
                    excluded_user_ids = []
    ...
    try:
        with get_db() as conn:
            ...
            if IS_PRODUCTION and DB_URL:
                ...
                if period == "weekly":
                    date_group = (
                        "TO_CHAR(DATE_TRUNC('week', date::date), 'YYYY-\"W\"WW')"
                    )
                    date_label = "Week"
                elif period == "monthly":
            ...
            for row in data:
                if isinstance(row, dict) or hasattr(row, "get"):
                    ...
                    if period_val is None or count_val is None:
                        ...
                        if len(keys) >= 2:
                            period_val = (
                                row.get(keys[0]) if period_val is None else period_val
                            )
                            count_val = (
                                row.get(keys[1]) if count_val is None else count_val
                            )
                else:
                    # SQLite returns tuples
...
async def get_geographic_analytics(
    ...
    try:
        with get_db() as conn:
            ...
            for row in city_results:
                if isinstance(row, dict):
                    ...
                    # SQLite returns tuples
                    city_data = {"city": row[0], "state": row[1], "count": row[2]}
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch geographic analytics: {str(e)}"
        )
...
async def get_category_cloud_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch category cloud data: {str(e)}"
        )
...
async def get_event_locations(
    ...
    try:
        with get_db() as conn:
            ...
            avg_lat = (
                sum(loc["lat"] for loc in locations) / total_locations
                if total_locations > 0
                else 0
            )
            avg_lng = (
                sum(loc["lng"] for loc in locations) / total_locations
                if total_locations > 0
                else 0
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch event locations: {str(e)}"
        )


# Page Visit Tracking - Privacy-friendly without personal data storage
async def track_page_visit(
    page_type: str, page_path: str, user_id: int = None, browser_fingerprint: str = None
):
    """
...
async def track_page_visit_endpoint(
    ...
    try:
        ...
        visit_tracked = await track_page_visit(
            page_type, page_path, user_id, browser_fingerprint
        )

        return {"success": True, "visit_tracked": visit_tracked, "page_type": page_type}
    ...
    try:
        with get_db() as conn:
            ...
            if excluded_user_ids:
                ...
                where_conditions.append(
                    f"(user_id IS NULL OR user_id NOT IN ({user_placeholders}))"
                )
                params.extend(excluded_user_ids)
            ...
            if IS_PRODUCTION and DB_URL:
                ...
                if period == "daily":
                    ...
                    date_group = (
                        "TO_CHAR(DATE_TRUNC('week', visited_at), 'YYYY-\"W\"WW')"
                    )
                    date_label = "Weekly"
                elif period == "monthly":
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch page visits analytics: {str(e)}"
        )
...
async def report_event(report_data: dict):
    ...
    try:
        ...
        try:
            ...
            report_reason = report_data.get(
                "reason", report_data.get("category", "Unknown")
            )
            subject = (
                f"Event Report: {report_reason} - Event ID {report_data['eventId']}"
            )
        ...
        # Log the report
        logger.info(
            f"Event report received - Event ID: {report_data['eventId']}, Type: {report_reason}, Reporter: {email}"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to submit report. Please contact support@todo-events.com directly.",
        )
...
async def submit_privacy_request(request: PrivacyRequest):
    ...
    try:
        ...
        logger.debug(
            f"Request data: request_type={request.request_type}, email={request.email}"
        )
        ...
        with get_db() as conn:
            ...
            if IS_PRODUCTION and DB_URL:  # PostgreSQL
                logger.debug(
                    f"Using PostgreSQL, inserting privacy request for {request.email}"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"privacy_requests table doesn't exist, creating it: {table_error}"
                    )
                ...
                result = cursor.fetchone()
                logger.debug(
                    f"PostgreSQL insert result: {result}, type: {type(result)}"
                )
                ...
                if result:
                    if hasattr(result, "keys") and hasattr(result, "values"):
                        ...
                        try:
                            ...
                            request_id = (
                                result.get("id") if hasattr(result, "get") else None
                            )
                    else:
                        request_id = None
                    ...
                    if not request_id:
                        ...
                        raise ValueError(
                            "Failed to get ID from PostgreSQL RETURNING clause"
                        )
                else:
                ...
                logger.debug(
                    f"Using SQLite, inserting privacy request for {request.email}"
                )
                cursor.execute(
                    f"""
                    INSERT INTO privacy_requests 
            ...
            if not request_id or request_id == 0:
                ...
                raise ValueError(
                    f"Failed to get valid ID for created privacy request, got: {request_id}"
                )

            conn.commit()

            logger.info(
                f"Privacy request {request_id} created successfully for {request.email} - type: {request.request_type}"
            )
            ...
            try:
                ...
                email_service.send_privacy_request_email(
                    request.email, request.request_type, request_id, user_details
                )
                logger.info(
                    f"Privacy request confirmation email sent for request {request_id}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to send privacy request confirmation email for request {request_id}: {e}"
                )
...
async def get_user_data_export(email: str, verification_code: str = None):
    ...
    try:
        with get_db() as conn:
            ...
            placeholder = get_placeholder()
            cursor.execute(
                f"SELECT id, email, role, created_at FROM users WHERE email = {placeholder}",
                (email,),
            )
            ...
            if user_row:
                ...
                user_data["page_visits"] = [
                    {"page_type": row[0], "page_path": row[1], "visited_at": row[2]}
                    for row in visits
                ]
    ...
    try:
        with get_db_transaction() as conn:
            ...
            if user_row:
                ...
                cursor.execute(
                    f"DELETE FROM event_interests WHERE user_id = {placeholder}",
                    (user_id,),
                )
                ...
                # Delete user account
                cursor.execute(
                    f"DELETE FROM users WHERE id = {placeholder}", (user_id,)
                )
        ...
        with get_db() as conn:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Both privacy request queries failed: {query_error}, {alt_error}"
                    )
                    return []
...
async def update_privacy_request_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to update privacy request status"
        )
    ...
    try:
        with get_db() as conn:
            ...
            stats["recent_uploads"] = (
                0  # Will be updated when forensic data is migrated
            )
            stats["storage_used_bytes"] = (
                0  # Will be calculated when forensic data is migrated
            )
...
async def flag_media(
    event_id: int, flag_data: dict, current_user: dict = Depends(get_current_user)
):
        ...
        with get_db() as conn:
            ...
            if action == "remove":
                if media_type == "banner":
                    c.execute(
                        f"UPDATE events SET banner_image = NULL WHERE id = {placeholder}",
                        (event_id,),
                    )
                elif media_type == "logo":
                    c.execute(
                        f"UPDATE events SET logo_image = NULL WHERE id = {placeholder}",
                        (event_id,),
                    )
                conn.commit()
...
async def get_user_forensics(
    ...
    try:
        with get_db() as conn:
            ...
            offset = (page - 1) * limit
            query += (
                f" ORDER BY u.created_at DESC LIMIT {placeholder} OFFSET {placeholder}"
            )
            params.extend([limit, offset])
...
async def get_comprehensive_audit_trail(
    ...
    try:
        with get_db() as conn:
            ...
            offset = (page - 1) * limit
            query += (
                f" ORDER BY al.timestamp DESC LIMIT {placeholder} OFFSET {placeholder}"
            )
            params.extend([limit, offset])
...
async def get_route_events_batch(request: RouteEventRequest):
    ...
    try:
        with get_db() as conn:
            ...
            if (
                request.dateRange
                and request.dateRange.get("startDate")
                and request.dateRange.get("endDate")
            ):
            ...
            for event in events:
                try:
                    ...
                    if event_id and event_id not in seen_event_ids:
                        ...
                        event_dict["view_count"] = int(
                            event_dict.get("view_count", 0) or 0
                        )
                    ...
                    logger.warning(
                        f"Error processing route event {event}: {event_error}"
                    )
                    continue

            logger.info(
                f"Route batch query returned {len(result)} unique events for {len(request.coordinates)} coordinates"
            )
            return result
