    ...
    <div className="space-y-2" ref={containerRef}>
      <form onSubmit={handleManualSubmit} className="relative">
        ...
        <input
          ...
          className={`w-full pl-10 pr-4 py-2 rounded-md border focus:ring-2 transition-all ${
            theme === 'light' 
              ? 'bg-white border-gray-300 text-gray-900 placeholder:text-gray-500 focus:ring-blue-500 focus:border-blue-500'
              : 'bg-gray-800 border-gray-600 text-gray-100 placeholder:text-gray-400 focus:ring-blue-400 focus:border-blue-400'
          }`}
        ...
        <button 
          type="submit"
          className={`absolute right-3 top-1/2 transform -translate-y-1/2 rounded-md px-2 py-1 text-xs ${
            theme === 'light'
      ...
      {showPredictions && predictions.length > 0 && (
        <div className={`absolute z-50 w-full border rounded-md shadow-lg mt-1 max-h-[200px] overflow-y-auto ${
          theme === 'light' ? 'bg-white border-gray-300' : 'bg-gray-800 border-gray-600'
        }`}>
          {predictions.map((prediction, index) => (
            <div
              key={prediction.place_id}
              className={`p-3 cursor-pointer first:border-t-0 ${
                theme === 'light' ? 'border-t border-gray-200' : 'border-t border-gray-700'
              } ${
                index === selectedIndex 
                  ? theme === 'light' ? 'bg-blue-50 text-gray-900' : 'bg-blue-900/20 text-gray-100'
                  : theme === 'light' ? 'hover:bg-gray-50 text-gray-900' : 'hover:bg-gray-700 text-gray-100'
              }`}
      ...
      {isLoading && (
        <div className={`text-xs ${theme === 'light' ? 'text-blue-500' : 'text-blue-400'}`}>Searching addresses...</div>
      )}
