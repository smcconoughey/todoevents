...
const createIconOnlyMarkerSVG = (iconPath, categoryColor, theme = THEME_DARK) => {
  ...
  const outlineWidth = isDarkMode ? '3' : '4'; // Slightly thicker outline for light mode
  const shadowColor = isDarkMode ? 'rgba(0, 0, 0, 0.5)' : 'rgba(255, 255, 255, 0.8)';
  
  return `
    <svg width="32" height="32" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <!-- Subtle background circle for better contrast -->
      <circle 
        cx="12" 
        cy="12" 
        r="11" 
        ...
        opacity="0.3"
      />
      <!-- Main icon with category color and outline -->
      ...
      <!-- Additional outline for better visibility -->
      <g transform="translate(0, 0)">
        ${iconPath.replace(
          /stroke="white"/g, 
          `stroke="${outlineColor}" stroke-width="1" opacity="0.8"`
        ).replace(
          /fill="white"/g, 
          `fill="none"`
        )}
...
// Icon paths from categoryConfig.js (duplicated here for independence)
const iconPaths = {
...
export const createIconOnlyMarker = (category, theme = THEME_DARK) => {
  ...
  return {
    ...
    optimized: false // Better rendering for complex SVGs
  };
};
