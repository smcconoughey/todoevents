...
const MissionOpsGrid = () => {
  ...
  if (isLoading) {
    return (
      <div className={`h-screen ${theme === 'light' ? 'light' : 'bg-neutral-950'} flex items-center justify-center`}>
  ...
  return (
    <div className={`h-screen ${theme === 'light' ? 'light' : 'bg-neutral-950 text-white'} relative overflow-hidden`}>
      {/* Header */}
      <div className={`absolute top-0 left-0 right-0 z-50 ${theme === 'light' ? 'light' : 'bg-neutral-900/80 border-neutral-700'} backdrop-blur-sm border-b`}>
      ...
      {/* Legend */}
      <div className={`absolute bottom-4 left-4 ${theme === 'light' ? 'light' : 'bg-neutral-900/90'} backdrop-blur-sm rounded-lg p-4 text-sm pointer-events-auto`}>
